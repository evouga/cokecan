...
public:
    QuadraticExpansionShellEnergy(
        ...
        bendingMatrix<LibShell::MidedgeAngleTanFormulation>(mesh, restPos, restEdgeDOFs, restState, bendingMcoeffs_);
        ...
        std::vector<Eigen::Triplet<double> >* hessian) const
    {
        ...
        if (!bendingOnly)
            result += LibShell::ElasticShell<LibShell::MidedgeAngleTanFormulation>::elasticEnergy(mesh_, curPos, curEdgeDOFs, mat_, restState_, LibShell::ElasticShell<LibShell::MidedgeAverageFormulation>::EnergyTerm::ET_STRETCHING, derivative, hessian);
...
public:
        ...
        std::vector<Eigen::Triplet<double> >* hessian) const
    {
        ...
        if (derivative)
        {
            derivative->resize(3 * nverts + curEdgeDOFs.size());
            derivative->setZero();
        }
        ...
        if (derivative)
        {
            derivative->segment(0,3*nverts) += bendingM_ * displacement;
        }
        if (hessian)
        {
            for (auto it : bendingMcoeffs_)
